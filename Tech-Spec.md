...
- Add a Chapter 8 knowledge-base learning application outside the reusable framework package. It composes `RAGTool`, `MemoryTool`, and the configured LLM into multi-format document ingestion, source-backed question answering, notes, recall, statistics, reports, and a session-isolated Gradio UI.
...
- Current memory truth: Working Memory and persistent Episodic Memory participate. Learning notes are stored as typed Episodic events until a real SemanticMemory implementation exists; Perceptual Memory, consolidation, and selective forgetting are not presented as implemented application features.
