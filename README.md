...
这是一个基于本地 HelloAgents 实践框架构建的多用户智能专家平台。它通过
`python -m apps.pdf_learning_assistant` 启动 Gradio 界面，提供本地账号、不同种类专家的权限边界、文档索引、基于来源的问答和询问统计。
...
| 范围 | 可见对象 | 当前版本允许的操作 |
| --- | --- | --- |
| `共享专家库` | 所有已登录用户 | 读取、上传文档、删除文档、保存笔记、问答和学习回顾 |
| 用户的专属专家 | 仅专属专家的所有者 | 创建、读取、上传、删除文档、保存笔记、问答、学习回顾和删除整个专家库 |
| `所有专家` | 当前用户可访问的通用专家库和专属专家库 | 只读聚合视图，用于汇总浏览文档和笔记；不能直接上传，问答必须选择一个具体专家库 |
...
- 笔记保存到当前选中的专家库，并作为 episodic memory 事件记录。
- 支持按内容搜索笔记和切换时间排序。
- 学习统计展示当前会话的会话时长、加载文档数、问答数、笔记数、当前专家库和当前文档。
- “总结本次会话真实问答”只总结当前登录会话中实际发生的问答，按涉及的专家库分组，不把文档内容、笔记或旧会话伪装成问答。
