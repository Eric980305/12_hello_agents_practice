...
- The monthly report path is covered by a regression test with real recorded turns from two knowledge bases. It requires two knowledge-base-labeled sections and excludes document-load events as invented dialogue input.
- The corrected selector was verified with a real temporary account: a fresh account exposed `所有知识库` and `共享知识库`; after creating a personal library it exposed all three choices and selected the new concrete library. The aggregate view retained the personal document, while upload, retrieval, and reporting stayed scoped to its concrete namespace.
